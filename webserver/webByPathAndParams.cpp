...
void handle_home(int clientSocket)
{
...
void handle_about(int clientSocket,const std::string& query_string)
{
    ...
    std::string response = "HTTP/1.1 200 OK\r\n"
                           ...
                           "\r\n"
                           "<h1>About Us</h1>"
                           "<p>We are a company specializing in web services.</p>\n"
                           "Parameters:\n";
...
void handle_not_found(int clientSocket)
{
...
void handle_request(int clientSocket)
{
    ...
    if (path == "/") {
        handle_home(clientSocket);
    } else if (path == "/about") {
        handle_about(clientSocket,query_string);
    } else {
        handle_not_found(clientSocket);
    }
